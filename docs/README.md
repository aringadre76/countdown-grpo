# Documentation

| File | Use it for |
|---|---|
| [`../README.md`](../README.md) | Public summary, current result, and short reproduction path. |
| [`experiment-protocol.md`](experiment-protocol.md) | Locked methods, gates, and decision rules. |
| [`amendment-2026-09-18.md`](amendment-2026-09-18.md) | Authorized learning-study alternatives, budget, dev gates, and confirmation analysis. |
| [`amendment-2026-09-22-sft-init-grpo.md`](amendment-2026-09-22-sft-init-grpo.md) | Frozen SFT-initialized binary-GRPO follow-up design, signal gates, confirmation, and remaining-budget rule. |
| [`amendment-2026-09-23-verifier-search.md`](amendment-2026-09-23-verifier-search.md) | Frozen inference-time exact-verifier search comparison; no training or change to historical GRPO claims. |
| [`reproduction.md`](reproduction.md) | Exact commands for a clean bounded rerun. |
| [`hardware.md`](hardware.md) | Intended hardware, observed WSL ROCm device, and compatibility caveats. |
| [`../artifacts/rechecks/2026-09-22-confirmation/README.md`](../artifacts/rechecks/2026-09-22-confirmation/README.md) | Frozen three-seed confirmation, actual compute, hardware, results, and artifact map. |
| [`../artifacts/rechecks/2026-09-22-confirmation/report-final/`](../artifacts/rechecks/2026-09-22-confirmation/report-final/) | Generated task-paired comparisons, trace audits, and plot. |
| [`../artifacts/rechecks/2026-09-22-sft-init-grpo/README.md`](../artifacts/rechecks/2026-09-22-sft-init-grpo/README.md) | Frozen design, commands, hardware, full compute accounting, result, and evidence map for the completed SFT-initialized GRPO follow-up. |
| [`../artifacts/rechecks/2026-09-22-sft-init-grpo/report-final/`](../artifacts/rechecks/2026-09-22-sft-init-grpo/report-final/) | Generated three-seed paired comparisons, bootstrap intervals, per-seed audit traces, and plot for SFT versus GRPO. |
| [`../artifacts/rechecks/2026-09-23-verifier-search/`](../artifacts/rechecks/2026-09-23-verifier-search/) | Frozen design and, when complete, task manifest, candidate-bank evaluations, and report for the verifier-filtered inference experiment. |
| [`lessons.md`](lessons.md) | Durable constraints and mistakes not to repeat. |
| [`next-steps.md`](next-steps.md) | Gated GPU recovery and experiment plan. |
| [`../CHANGELOG.md`](../CHANGELOG.md) | Intentional project changes by release. |
| [`../AGENTS.md`](../AGENTS.md) | Routing and safety instructions for coding agents. |
| [`.agents/skills/learn-commit-push`](../.agents/skills/learn-commit-push/SKILL.md) | Project-local validate, commit, push, and continuity workflow. |
| [`codex-goal.md`](codex-goal.md) | Compact Codex execution prompt derived from the protocol. |

The README is the public entry point. The protocol and saved artifacts are the
source of truth for methods and results; update prose only from those records.
